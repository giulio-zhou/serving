...
class MnistServiceImpl final {
 public:
   MnistServiceImpl(const string& name);
...
MnistServiceImpl::MnistServiceImpl(const string& name) : name_(name) {
  ...
  // scheduler_options.num_batch_threads = 16;
...
void MnistServiceImpl::Classify(CallData* calldata) {
  for (int c = 0; c < kNumLabels; ++c) {
    calldata->mutable_response()->add_value(1.1*c);
  }
...
void RunServer(const int port) {
  ...
  MnistServiceImpl service_impl("crankshaw_server");
...
int main(int argc, char** argv) {
  ...
  if (argc != 1) {
    LOG(FATAL) << "Usage: mnist_grpc_server --port=9000";
  }
